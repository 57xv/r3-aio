...
class NintendoBypass:
    def __init__(self):
        self.api_base = "https://ka.idarko.xyz"
        self.auth_url = "https://accounts.nintendo.com/connect/1.0.0/authorize?client_id=1f6a6a4806931686&redirect_uri=https://accounts.epicgames.com/OAuthAuthorized&response_type=id_token&scope=openid&state=STATE"
        self.auth_type = "nintendo_id_token"
        self.token_param = "id_token"
        
    def start(self):
        Title("Nintendo Bypasser")
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Initializing Nintendo Auth Flow (Darky API)...")
        
        print(f"\n{BEFORE + current_time_hour() + AFTER} {WAIT} Perform the following steps:")
        print(f"   1. Browser will open automatically.")
        print(f"   2. Log in to your Nintendo account.")
        print(f"   3. Right-click 'Select this person' and copy the link address.")
        print(f"      (OR copy the final redirect URL if redirected)")
            ...
            if data.get('success'):
                ...
                if save_choice == 'y':
                    ...
                    if token_data and 'access_token' in token_data:
                        ...
                        if device_auth:
                            print(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Enter Account Name (e.g. 'My Nintendo'):")
    ...
    def save_results(self, code):
        results_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Results', 'Nintendo Bypasser')
